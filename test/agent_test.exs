defmodule AgentTest do
  ...
  test "start_link/2 workflow with unregistered name" do
    ...
    assert Agent.get_and_update(pid, &Map.pop(&1, :hello)) == :world
  ...
  test "start/2 workflow with registered name" do
    ...
    assert Agent.update(:agent, &Map.put(&1, :hello, :world)) == :ok
    assert Agent.get(:agent, &Map.get(&1, :hello), 3000) == :world
